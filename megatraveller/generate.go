...
// column of the starport table (MegaTraveller Basic Mainworld Generation).
type Nature int
...
// rollGasGiants rolls gas-giant presence (2D >= 5) and, when present, a 2D
// quantity. When absent, the quantity die is not rolled.
func rollGasGiants(r dice.Roller) int {
	if r.D6(2) < 5 {
		return 0
	}
